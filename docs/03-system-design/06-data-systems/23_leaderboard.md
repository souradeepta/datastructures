...
**Status:** draft
**Audience:** Backend engineer preparing for an L4–L5 ranking and real-time data interview.
**Prerequisites:** ordered indexes, sharding, consistency, pagination, and event processing.
...
**Calculations:**
```
Assume 1 million players submit 2 score updates/minute and the service serves 5,000 reads/s. Separate write ordering, top-K maintenance, rank queries, and snapshot reads; a sorted structure's asymptotic cost does not establish a p99 SLO.
...
If a page contains 100 entries at 64 bytes each, one leaderboard response is about 6.4 KB before metadata; cache stable pages while defining how quickly a score update becomes visible.
```
