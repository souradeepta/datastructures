...
**Status:** draft
**Audience:** Backend engineer preparing for an L4–L5 financial-data and consistency interview.
**Prerequisites:** transactions, double-entry ledgers, idempotency, concurrency, and auditability.
...
**Calculations:**
```
Assume 100,000 wallets and 2,000 transfers/s at peak. Serialize updates per account or use optimistic version checks; balance reads can be cached only with a stated freshness policy and must not authorize a withdrawal from stale state.
...
If each transfer creates two ledger entries and a 1 KB audit event, 2,000 transfers/s produce about 6 MB/s of raw records before replication and indexes; retain immutable entries for reconciliation.
```
