...
**Status:** draft
**Audience:** Engineer preparing for an L3–L5 object-oriented design interview.
**Prerequisites:** interfaces, composition, dependency injection, testing, and runtime configuration.
...
**Calculations:**
```
Assume a service selects one parser for 2,000 files/s, with 80% CSV, 15% JSON, and 5% XML. The factory's decision rate is 2,000 selections/s; parser throughput, memory, and error behavior must be measured per format.
...
If parser construction takes 2 ms and each request constructs a parser, it consumes 4 CPU-seconds/s at 2,000 files/s; reuse stateless parsers or pool expensive resources only after measuring lifecycle and thread-safety constraints.
```
