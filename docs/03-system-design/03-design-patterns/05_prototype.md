...
**Status:** draft
**Audience:** Engineer preparing for an L3–L5 object-oriented design interview involving snapshots or expensive object creation.
**Prerequisites:** copying, immutability, object graphs, serialization, and ownership of mutable state.
...
**Calculations:**
```
Assume a 2 MB configuration graph is cloned for 100 tenant variants/minute. Deep copying moves about 200 MB/minute before allocator overhead; compare that with rebuilding from a source of truth and verify that secrets, sockets, and caches are not copied accidentally.
...
If 10,000 snapshots are retained at 2 MB each, snapshot storage is about 20 GB before compression and metadata; define retention and copy-on-write behavior explicitly.
```
