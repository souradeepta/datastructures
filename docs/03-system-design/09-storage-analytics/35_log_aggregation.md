...
**Status:** draft
**Audience:** SRE or data engineer preparing for an L4–L5 observability-storage interview.
**Prerequisites:** log pipelines, queues, indexing, retention, sampling, PII handling, and search.
...
**Calculations:**
```
Assume 10,000 hosts emit 2,000 log lines/s at 500 bytes each. Ingest is about 10 GB/s before compression; batching, backpressure, dropped-debug policy, replication, and indexing must be sized separately.
...
At 10 GB/s sustained, one hour contains about 36 TB of raw log payload. A 5:1 compression estimate yields 7.2 TB before indexes and replicas, and must be verified against actual log entropy.
```
